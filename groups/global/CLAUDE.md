...
You have persistent memory via the `mem0` skill that survives across sessions:

- Use `mem0 recall "<topic>"` at the start of conversations to load relevant context
- Use `mem0 remember "<fact>"` when you learn important facts about the user, project, or team
- Use `mem0 recall "<topic>" --user team` for shared team knowledge
- Your memories persist across sessions — you will remember past interactions

When the user tells you something worth remembering (preferences, decisions, context), store it with `mem0 remember`. When you need context from past work, search with `mem0 recall`.
